...
pub fn get_ui_list<'a>(
	state: &'a ActiveList,
	autocomplete: &'a Option<Autocomplete>,
	fuzzyfind: &'a List,
) -> &'a List {
	match state {
		ActiveList::Autocomplete => {
			if let Some(autocomplete) = autocomplete.as_ref() {
				&autocomplete.list
			} else {
				&None
			}
...
pub fn handle_update_placeholder(
	...
	if let Some(pt) = passthrough {
		// reset passthrough if it is no longer valid
		if !pt.get_factory().should_create(&search) {
			*passthrough = None;
		}
	} else {
		for factory in passthrough_factories.iter() {
			// look for a passthrough to use
...
pub fn passthrough_command(
	...
	if let Some(passthrough) = passthrough.as_ref() {
		let (passthrough_command, passthrough_base_command, passthrough_command_type) =
			passthrough.get_command();
		...
		(
			format!("{}{}", search, passthrough_command),
			passthrough_base_command,
